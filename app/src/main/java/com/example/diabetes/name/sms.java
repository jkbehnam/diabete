...
public class sms {
    ...
    public sms(StringBuilder sb, String a, Context bx) {
        s = a;
        cx = bx;
        stb = sb;
    }
    ...
    public void sendSMSMessage() {
        ...
        SQLiteDatabase db = openOrCreateDatabase(User_Path + "profile_info.db",
                null, null);
        cr = db.rawQuery("select phone_1,phone_2,send_check_warning from message", null);
        cr_name = db.rawQuery("select fname,lname from profile_info1", null);
            ...
            if (cr.getInt(2) == 1) {
                ...
                if (!(cr.getString(0).equals(""))) {
                    if (cr_name.getCount() != 0 && !(cr_name.getString(0).equals("")) && !(cr_name.getString(1).equals(""))) {
                        Toast.makeText(cx, "پیام هشدار اول  ارسال شد", Toast.LENGTH_SHORT).show();
