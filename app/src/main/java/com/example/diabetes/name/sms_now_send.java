 ...
 */
public class sms_now_send {
    String s;
    Context cx;
    StringBuilder stb;

    public sms_now_send(StringBuilder sb, String a, Context bx) {
        s = a;
        cx = bx;
        stb = sb;
    }
    ...
    public void sendSMSMessage() {
        ...
        SQLiteDatabase db = openOrCreateDatabase(User_Path + "profile_info.db",
                null, null);
        Cursor cr = db.rawQuery("select phone_1,phone_2,send_check_warning from message", null);
            ...
            if (true) {
                ...
                if (!(cr.getString(0).equals(""))) {
                    if (cr_name.getCount() != 0 &&!(cr_name.getString(0).equals("")) && !(cr_name.getString(1).equals(""))) {
                        Toast.makeText(cx, "پیام  اول  ارسال شد", Toast.LENGTH_SHORT).show();
                        ...
                        Toast.makeText(cx, "برای ارسال پیام  باید نام کامل وارد شود", Toast.LENGTH_SHORT).show();
                    }
                } else
                    Toast.makeText(cx, "شماره موبایل برای ارسال پیام  وارد نشده", Toast.LENGTH_SHORT).show();
                if (!(cr.getString(1).equals(""))) {
                    if (cr_name.getCount() != 0 &&!(cr_name.getString(0).equals("")) && !(cr_name.getString(1).equals(""))) {
                        Toast.makeText(cx, "پیام هشدار دوم  ارسال شد", Toast.LENGTH_SHORT).show();
