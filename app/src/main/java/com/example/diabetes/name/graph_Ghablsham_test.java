...
public class graph_Ghablsham_test extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ghablsham);

        overridePendingTransition(R.anim.anim_notmain2, R.anim.anim_notmain1);

        toolbar = (Toolbar) findViewById(R.id.ghablsham);
        ...
        final GraphView graph = (GraphView) findViewById(R.id.graph_ghablsham);



        graph.setTitle("نمودار قند قبل از شام");

        SQLiteDatabase db = openOrCreateDatabase(User_Path + "profile_info.db",
                SQLiteDatabase.CREATE_IF_NECESSARY, null);
        final Cursor cr = db.rawQuery("select * from glucose_tb  where when_d ='"+4+"' order by glu_y , glu_m , glu_d ", null);
...
//--------------------------
                if (ss <= p) {
                    ...
                    JalaliCalendar dateandtime2 = new JalaliCalendar(cr.getInt(cr.getColumnIndex("o_y")), cr.getInt(cr.getColumnIndex("o_m")), cr.getInt(cr.getColumnIndex("o_d")));
            ...
            series_khatar2.setTitle("مقدار خطر");
