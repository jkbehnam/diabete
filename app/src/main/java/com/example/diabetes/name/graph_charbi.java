...
public class graph_charbi extends BaseActivity2 {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_charbi);

        toolbar = (Toolbar) findViewById(R.id.charbiappbar);
        setSupportActionBar(toolbar);
        TextView txttoolbar = (TextView) findViewById(R.id.txttoolbar);
        txttoolbar.setText("نمودار چربی خون");
       ...
       final GraphView graph = (GraphView) findViewById(R.id.charbigraph);


        graph.setTitle("نمودار چربی (ماهیانه)");

        SQLiteDatabase db = openOrCreateDatabase(User_Path + "profile_info.db",
                SQLiteDatabase.CREATE_IF_NECESSARY, null);
        final Cursor cr = db.rawQuery("select * from fat_tb order by o_y ,o_m , o_d  ", null);
