...
public class graph_Ghablazsobh extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ghablazsobh);

        overridePendingTransition(R.anim.anim_notmain2, R.anim.anim_notmain1);

        toolbar = (Toolbar) findViewById(R.id.ghablsobhane);
        ...
        txttoolbar.setText("قبل از صبحانه");
        ...
        final GraphView graph = (GraphView) findViewById(R.id.graph_ghablsobhane);

        graph.setTitle("نمودار قند قبل از صبحانه");

        SQLiteDatabase db = openOrCreateDatabase(User_Path + "profile_info.db",
                SQLiteDatabase.CREATE_IF_NECESSARY, null);
        final Cursor cr = db.rawQuery("select * from glucose_tb  where when_d ='"+0+"' order by glu_y , glu_m , glu_d ", null);
...
//--------------------------
                ...
                if (ss <= p) {
                    ...
                    JalaliCalendar dateandtime2 = new JalaliCalendar(cr.getInt(cr.getColumnIndex("glu_y")), cr.getInt(cr.getColumnIndex("glu_m"))-1, cr.getInt(cr.getColumnIndex("glu_d")));
