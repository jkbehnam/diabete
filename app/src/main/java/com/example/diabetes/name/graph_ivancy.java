...
public class graph_ivancy extends BaseActivity2 {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ivancy);



        toolbar=(Toolbar)findViewById(R.id.ivancyappbar);
        setSupportActionBar(toolbar);
        TextView txttoolbar=(TextView)findViewById(R.id.txttoolbar);
        txttoolbar.setText("نمودار قند سه ماهه");
        ...
        final GraphView graph = (GraphView) findViewById(R.id.ivancygraph);



        graph.setTitle("نمودار قند سه ماهه");

        SQLiteDatabase db = openOrCreateDatabase(User_Path + "profile_info.db",
                SQLiteDatabase.CREATE_IF_NECESSARY, null);
        final Cursor cr = db.rawQuery("select * from ivancy_tb order by o_y ,o_m , o_d  ", null);
