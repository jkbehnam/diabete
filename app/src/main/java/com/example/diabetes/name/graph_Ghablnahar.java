...
public class graph_Ghablnahar extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ghablnahar);

        overridePendingTransition(R.anim.anim_notmain2, R.anim.anim_notmain1);

        toolbar = (Toolbar) findViewById(R.id.ghablnahar);
        ...
        txttoolbar.setText("قبل از ناهار");
        ...
        final GraphView graph = (GraphView) findViewById(R.id.graph_ghablnahar);



        graph.setTitle("نمودار قند قبل از نهار");

        SQLiteDatabase db = openOrCreateDatabase(User_Path + "profile_info.db",
                SQLiteDatabase.CREATE_IF_NECESSARY, null);
        final Cursor cr = db.rawQuery("select * from glucose_tb  where when_d ='"+2+"' order by glu_y , glu_m , glu_d ", null);
